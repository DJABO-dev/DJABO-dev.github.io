...
#----------------------------- FUNCIÓN PARA OBTENER LAS UNIDADES USB Y DISCOS DUROS (SOLO LINUX) ----------------------------
# Obtener las unidades USB montadas en el sistema
def obtener_unidades_usb():
    ...
    unidades = subprocess.run(['df', '-h'], capture_output=True, text=True)
    entradas = unidades.stdout.splitlines()
    for entrada in entradas[1:]:
        seccion = entrada.split()
        if len(seccion) > 0 and  ((seccion[5].startswith('/media')) or (seccion[5].startswith('/mnt'))):
            unidades_usb.add(seccion[5])
            unidades_usb.add('/home/japimaster/Nextcloud/Xarchivo/ENGINE_DJ')                   
...
# OBTENER HISTORIAL DE hm.db
    resultados_history = []
    db_history = "/Engine Library/Database2/hm.db"
    frase_history = valor + db_history
    if os.path.isfile(frase_history):
        conexion_history = sqlite3.connect(frase_history)
        cursor_history = conexion_history.cursor()
        cursor_history.execute("SELECT startTime, timezone FROM Historylist")
        resultados_history = cursor_history.fetchall()
...
# SI LA BASE ESTÁ VACÍA
        ...
        if resultados_history:
            for startTime, timezone in resultados_history:
                numeric_data = int(startTime)
                fecha = datetime.datetime.fromtimestamp(numeric_data)
                tree_left.insert("", 'end', text='[❇] HISTORY '+str(fecha))
...
## ASEGURARSE DE QUE LA BASE DE DATOS EXISTE EN LA UNIDAD
    ...
    if resultados_history:
        for startTime, timezone in resultados_history:
            numeric_data = int(startTime)
            fecha = datetime.datetime.fromtimestamp(numeric_data)
            tree_right.insert("", 'end', text='[❇] HISTORY '+str(fecha))
...
def mostrar_ventana_tracks_left():
	...
	if nodos_listar:
		...
		ventana_tracks.geometry("800x800")
...
#  FUNCIÓN PARA MOSTRAR UNA VENTANA CON LAS TRACKS DEL HISTORIAL
def mostrar_ventana_history(library_side):
	print(f'ventana history: {history_item}')
	fecha = history_item[12:]
	print(f'{library_side}')
	global ventana_history
	ventana_history = tk.Toplevel()
	ventana_history.title("HISTORIAL")
	ventana_history.geometry("700x800")
	frame_para_botonera = tk.Frame(ventana_history)
	frame_para_botonera.pack(fill=tk.BOTH, expand=False)
	button_guardar = tk.Button(frame_para_botonera, text="Guardar", state='active', command=lambda: guardar_historial(fecha, canciones))
	button_guardar.pack(side=tk.LEFT, padx=10, pady=10, anchor="w")
	
	button_Salir = tk.Button(frame_para_botonera, text="SALIR", state='active', command=ventana_history.destroy)
	button_Salir.pack(side=tk.LEFT, padx=10, pady=10, anchor="w")
	frame_para_lista = tk.Frame(ventana_history)
	frame_para_lista.pack(fill=tk.BOTH, expand=True)
	tree_tracks = ttk.Treeview(frame_para_lista, height=10, selectmode='extended')
	tree_tracks.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
	scrollbar = ttk.Scrollbar(frame_para_lista, orient="vertical", command=tree_tracks.yview)
	scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
	tree_tracks.configure(yscroll=scrollbar.set)
	history_database_path = library_side + "/Engine Library/Database2/hm.db"
	if os.path.isfile(history_database_path):
		history_connection = sqlite3.connect(history_database_path)
		cursor_history = history_connection.cursor()
		cursor_history.execute("SELECT id, startTime, timezone FROM Historylist")
		fechas = cursor_history.fetchall()
		tree_tracks.insert("", 'end', fecha, text='[❇] HISTORY: '+ fecha)
		for resultado in fechas:
			id, startTime, timezone = resultado
			startTime_transformado = int(startTime)
			fecha_resultado = datetime.datetime.fromtimestamp(startTime_transformado)
			print(f'FECHA RESULTADO: {fecha_resultado}')
			print(f'FECHA: {fecha}')
			
			# Convertir las cadenas a objetos datetime
			fecha_hora1 = datetime.datetime.strptime(str(fecha), "%Y-%m-%d %H:%M:%S")
			fecha_hora2 = datetime.datetime.strptime(str(fecha_resultado), "%Y-%m-%d %H:%M:%S")
		
			if fecha_hora1 == fecha_hora2:
				canciones = []
				print(f'COINCIDEN: ||{fecha}|| - ||{fecha_resultado}||')
				cursor_history.execute("SELECT trackId FROM HistorylistEntity WHERE listId = ?", (id,))
				tracks = cursor_history.fetchall()
				for tracks_in_history  in tracks:
					cursor_history.execute("SELECT title, artist FROM Track WHERE id = ?", (tracks_in_history))
					datos = cursor_history.fetchone()
					title = datos[0]
					artist = datos[0]
					tree_tracks.insert(fecha, 'end', text='🎝 '+ title + " 👷 " + artist)
					canciones.append((title, artist))
	cursor_history.close()
	history_connection.close()	

def guardar_historial(fecha, canciones):
	ventana_history.withdraw()
	print(f'guardando historial')
	archivo = filedialog.asksaveasfilename(
	defaultextension=".txt",  
	filetypes=[("Text files", "*.txt"), ("All files", "*.*")],  
	initialfile=(f'{fecha}.txt'),
	title="Guardar Historial como"  
	)
	if archivo:  
		with open(archivo, 'w') as f:
			f.write('Historial de fecha '+ fecha + '\n\n')
			for track in canciones:
				f.write(f'{track[0]} - {track[1]}\n')
            
		messagebox.showinfo("Historial Guardado", "Se ha guardado el historial en el archivo " + (f'{archivo}'))
	ventana_history.deiconify()
...
def arreglar_rojos(tracks_rojos, valor, ventana_tracks, funcion_original):
	...
	for track_rojo in tracks_rojos:
		for carpeta_actual, subcarpetas, archivos in os.walk(valor, followlinks=True):
...
def crear_ventana():
    ...
    ventana = tk.Tk()
    ventana.title("Engine DJ SYNCHRONIZE FOR LINUX version 1.1.2 BY DJ ABO")
...
# Crear los campos de texto con la información del USB seleccionado en el combobox
    ...
    button100 = tk.Button(frame_para_botones, state='disabled', text="HISTORY", command=lambda: mostrar_ventana_history(library_side)) 
    button100.pack(side=tk.LEFT, padx=10, pady=5, anchor="center")
